...
def download_partition_functions():
    """Download partition function files for N2O isotopologues"""
    os.makedirs('data', exist_ok=True)
    
    # N2O isotopologues use q21.txt, q22.txt, q23.txt, q24.txt, q25.txt
    q_files = [21, 22, 23, 24, 25]
...
def main():
    ...
    # Look for manually downloaded N2O HITEMP file
    N2O_file = None
    for fname in os.listdir('downloads'):
        if fname.endswith('.bz2') and ('N2O' in fname or fname.startswith('04_')):
            N2O_file = fname
            break
    
    if not N2O_file:
        print("ERROR: No N2O HITEMP .bz2 file found in downloads/")
        print("Please manually download N2O HITEMP 2019 data and place in downloads/")
        return
    
    # Decompress the HITEMP data
    with open(f'extract/{N2O_file[:-4]}.par', 'wb') as new_file, bz2.BZ2File(f'downloads/{N2O_file}', 'rb') as f:
